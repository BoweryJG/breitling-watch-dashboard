...
export class MetricsCalculator {
  ...
  static calculatePatientMetrics(patients: Patient[], testimonials: Testimonial[]): PatientMetrics {
    ...
    // Patient satisfaction average from testimonials
    const approvedTestimonials = testimonials.filter(t => t.is_approved);
    const satisfactionAverage = approvedTestimonials.length > 0
  ...
  static calculatePerformanceMetrics(
    ...
    const approvedTestimonials = testimonials.filter(t => t.is_approved);
    const testimonialRating = approvedTestimonials.length > 0
      ? approvedTestimonials.reduce((sum, t) => sum + t.rating, 0) / approvedTestimonials.length
      : 5;
  ...
  static calculateAllMetrics(
    ...
    return {
      ...
      patients: this.calculatePatientMetrics(patients, testimonials),
      services: this.calculateServiceMetrics(services, appointments),
      performance: this.calculatePerformanceMetrics(appointments, testimonials),
    };
  }
